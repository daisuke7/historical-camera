...
| P1 | 磨き込み: レンズ切替・ズーム・保存サムネイル・非線形スライダー調整・実機チューニング | 初回リリース版 |
| P2 | 録画(07 の設計を実装) | アップデート |
...
### T2. Dart ドメイン層
- `filter_params.dart`(FilterParams + `toMap()` + `lerp(a, b, t)`)
...
- 03 §3 を GLSL ES 移植(external/2D の 2 変種コンパイル)。
- [参照] 03, 06(§3.1, §4)
...
### T11. 結合・ライフサイクル
- background/復帰、権限拒否画面、エラーイベント表示、wakelock、白フラッシュ演出。
- [参照] 02 §1(設計原則4), 04 §5
...
1. Flutter SDK(stable 最新)+ Android Studio(Flutter/Dart プラグイン)
...
| リスク | 影響 | 兆候の検知 | 対応 |
|--------|------|-----------|------|
| Impeller と外部テクスチャの互換問題(Android) | プレビューが映らない | T8 のパススルー時点で判明 | Impeller 無効化(06 §8)。Flutter バージョン固定 |
...
| 1000 年分の表現の単調さ(1000-1500 が変化に乏しい) | 体験の間延び | 主観評価 | キーフレーム追加(例: 1200 年に顔料の変化)。スケール配分(04 §2.1)の調整 |
