...
## 8. デバッグ隠し画面(P1。チューニング用の開発者機能)

03 の値(キーフレーム表・シェーダー定数のうちパラメータ化されているもの)を実機で
確定させるための隠し画面。**リリースビルドにも含める**(導線が隠されているのみ —
08 §8.3)。

### 8.1 導線

- 年代ラベル(§3)を **3 秒長押し**で開く。通常操作(タップ・スライダー)とは衝突しない。

### 8.2 構成(全画面の半透明パネル、ダーク UI)

- **モード切替**: `年代連動(既定)/ 手動`。
  - 手動に切り替えた時点の `paramsForYear(year)` を手動値の初期値にする。
  - 手動中はパネルのスライダー値をそのまま `setFilterParams` へ送る
    (既存の 16ms スロットリング経路を共用)。
  - 手動中に背後の EraSlider を動かした場合は、その year の `paramsForYear` で
    手動値を再初期化する(「近い年代から始めて微調整」の作業動線)。
  - 年代連動に戻すと通常動作へ復帰する。
- **20 パラメータのスライダー**: 02 §2 の宣言順・値域。現在値を数値でも表示。
- **JSON コピー**: 現在の 20 値を 02 §2 のキー名の JSON としてクリップボードへコピー
  (03 §2.1 のキーフレーム表へ転記するための出力)。
- **GPU 時間表示**: パネル表示中は `setDebugStatsEnabled(true)` にし、`debugStats`
  イベント(02 §3.2)の gpuMs を表示する。8ms 超は赤表示(予算超過の即時検知)。
- **検証用トグル**: 解像度の明示指定(auto/hd720/hd1080 — 再 initialize を伴う)、
  版画の地域プリセット切替(和/洋 — 03 §5。実装後)。
- **閉じる**: パネル右上の ×。閉じても手動モードは維持し、画面上部に小さな
  `手動` バッジを表示する(タップで年代連動に復帰)。

### 8.3 実装方針

- `ui/debug_panel.dart` 1 ファイル+専用 provider。既存 `CameraScreen` への変更は
  導線(長押し検出)とバッジ表示のみに留める。
- 文言は `strings.dart` に集約(日本語のみでよい)。
- パネル表示中もプレビュー・撮影は通常どおり動作する(撮って確かめながら調整できる)。
